...
void loop() {
  ...
  if (tiempoActual - greenMillis >= greenDelay && green1Status == true && red2Status == true) {
  ...
  if (tiempoActual - orangeMillis >= orangeDelay && orange1Status == true) {
    Serial.println("Entra a amarillo");
    redMillis = millis();
    digitalWrite(o1Pin, LOW);
    digitalWrite(o2Pin, LOW);
    orange1Status = false;
  ...
  if (tiempoActual - green2Millis >= greenDelay && green2Status == true) {
    Serial.println("Entra a otro semaforo");
  ...
  if (tiempoActual - orange2Millis >= orangeDelay && orange2Status == true) {
    red2Millis = millis();
    digitalWrite(o3Pin, LOW);
    orange2Status = false;
  ...
  if (tiempoActual - redMillis >= redDelay && red1Status == true && red2Status == true) {
    vueltaMillis = millis();
    digitalWrite(r1Pin, LOW);
    red1Status = false;
